...
-- Parcelamento por compra (itens da fatura)
...
ALTER TABLE public.credit_card_invoice_items
  DROP CONSTRAINT IF EXISTS credit_card_invoice_items_installment_chk;

ALTER TABLE public.credit_card_invoice_items
  ADD CONSTRAINT credit_card_invoice_items_installment_chk CHECK (
    (installment_group_id IS NULL AND installment_number IS NULL AND installment_count IS NULL)
    OR (
      installment_group_id IS NOT NULL
      AND installment_number IS NOT NULL
...
CREATE INDEX IF NOT EXISTS credit_card_invoice_items_installment_group_idx
  ON public.credit_card_invoice_items (installment_group_id);
