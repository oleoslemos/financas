...
const linkClass = ({ isActive }: { isActive: boolean }) =>
  `rounded-lg px-3 py-2 text-xs font-medium tracking-wide ${isActive ? 'bg-slate-800 text-sky-300' : 'text-slate-400 hover:bg-slate-900 hover:text-slate-200'}`

export function AppLayout() {
  return (
    <div className="min-h-screen flex flex-col uppercase md:flex-row">
      ...
      <main className="max-w-6xl w-full flex-1 p-4 md:p-8">
        <Outlet />
      </main>
    </div>
  )
