...
export function V2CategoriesPage() {
  const { user } = useUser()
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4 bg-slate-900/60 backdrop-blur-md border border-slate-800/80 rounded-3xl p-6 shadow-xl">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-slate-400">· Organização & Classificação</span>
          </div>
          <h2 className="text-2xl sm:text-3xl font-black text-white tracking-tight">Categorias</h2>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-slate-500 text-xs">Nenhuma categoria cadastrada.</div>
      ) : (
